...
# Sort by city then by original order (hourly sequential data)
df = df.sort_values(['qyteti', 'month', 'day_of_week', 'hour']).reset_index(drop=True)

# Lag features per city (shift within each city group)
...
# Drop rows with NaN from lag (first 24 rows per city)
before_drop = len(df)
df = df.dropna().reset_index(drop=True)
print(f"   Lag features: fshire {before_drop - len(df)} rreshta (fillimi i serive)")
print(f"   Rreshta finalet: {len(df)}")
print(f"   Kolonat e reja: pm2_5_lag_1h, pm2_5_lag_24h, pm10_lag_1h, temp_x_wind, humidity_x_heating, temp_x_humidity")

# MODEL A: Me PM10 (baseline per krahasim)
...
# Temporal split for Model A too
split_idx_a = int(len(X_a) * 0.8)
X_train_a, X_test_a = X_a.iloc[:split_idx_a], X_a.iloc[split_idx_a:]
y_train_a, y_test_a = y.iloc[:split_idx_a], y.iloc[split_idx_a:]
...
# MODEL B: PA PM10 (modeli i vertete - pa data leakage)
print("MODEL B: PA PM10 (Modeli i vertete, pa data leakage)")
FEATURES_B = [col for col in df.columns if col not in [TARGET, 'pm10', 'pm10_lag_1h']]

X_b = df[FEATURES_B]

# Temporal split: last 20% as test (respects time order)
...
print(f"   Train: {len(X_train_b)}, Test: {len(X_test_b)} (temporal split)")
...
# Cross-validation for Model B (TimeSeriesSplit - respects temporal order)
...
# Feature Importance per MODEL B (modeli i vertete)
...
# --- Grafiku 1: Feature Importance (Model B - pa PM10) ---
...
ax.set_title('Feature Importance - PA PM10 (Modeli i vertete)', fontsize=14, fontweight='bold')
ax.invert_yaxis()
...
# --- Grafiku 2: Actual vs Predicted (Model B) ---
...
plt.suptitle('Krahasimi: Actual vs Predicted', fontsize=15, fontweight='bold')
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'rf_actual_vs_predicted.png'), dpi=150)
plt.close()

# --- Grafiku 3: Residuals per Model B ---
...
ax.set_title(f'Residuals Model B (Mean={residuals_b.mean():.2f}, Std={residuals_b.std():.2f})',
             fontsize=14, fontweight='bold')
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'rf_residuals.png'), dpi=150)
plt.close()

# --- Grafiku 4: Bar Chart krahasimi ---
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'rf_model_comparison.png'), dpi=150)
plt.close()
...
print(f"Modeli B u ruajt te: {MODEL_DIR}random_forest_pm25.joblib")
...
with open(results_path, 'w', encoding='utf-8') as f:
    ...
    f.write('MODEL A (me PM10 - baseline):\n')
    f.write(f'  MAE:  {mae_a:.4f}\n')
    f.write(f'  RMSE: {rmse_a:.4f}\n')
    f.write(f'  R²:   {r2_a:.4f}\n\n')
    f.write('MODEL B (pa PM10 - modeli kryesor):\n')
    f.write(f'  MAE:  {mae_b:.4f}\n')
    f.write(f'  RMSE: {rmse_b:.4f}\n')
    f.write(f'  R²:   {r2_b:.4f}\n\n')
    f.write(f'Cross-Validation (5-Fold):\n')
    f.write(f'  Mean R²: {cv_scores.mean():.4f} ± {cv_scores.std():.4f}\n\n')
    f.write('Feature Importance (Model B):\n')
    for _, row in feature_imp_df.iterrows():
...
rf_eval = {
    ...
    'feature_importance': {row['Feature']: round(row['Importance'], 4)
                           for _, row in feature_imp_df.iterrows()},
...
# Eksporti i parashikimeve (per krahasim grafik me modelet tjera)
pred_df = pd.DataFrame({
    'actual': y_test.values,
    'predicted_rf': y_pred_b
})
pred_path = os.path.join(MODEL_DIR, 'predictions_random_forest.csv')
pred_df.to_csv(pred_path, index=False)
print(f"Parashikimet: {pred_path}")
